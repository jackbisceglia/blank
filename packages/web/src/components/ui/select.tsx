...
export const SelectTrigger = <T extends ValidComponent = 'button'>(
  ...
  return (
    <SelectPrimitive.Trigger
      class={cn(
        'flex h-9 w-full items-center justify-between rounded-md border border-ui-input bg-ui-transparent px-3 py-2 text-sm shadow-sm ring-offset-ui-background transition-shadow placeholder:text-ui-muted-foreground focus:outline-none focus-visible:ring-[1.5px] focus-visible:ring-ui-ring disabled:cursor-not-allowed disabled:opacity-50',
        local.class,
      )}
      {...rest}
    >
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        class={cn(
          'relative z-50 min-w-[8rem] overflow-hidden rounded-md border bg-ui-popover text-ui-popover-foreground shadow-md data-[expanded]:animate-in data-[closed]:animate-out data-[closed]:fade-out-0 data-[expanded]:fade-in-0 data-[closed]:zoom-out-95 data-[expanded]:zoom-in-95',
          local.class,
        )}
        {...rest}
      >
