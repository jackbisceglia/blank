...
export const ComboboxInput = <T extends ValidComponent = 'input'>(
  ...
  return (
    <ComboboxPrimitive.Input
      class={cn(
        'h-full bg-ui-transparent text-sm placeholder:text-ui-muted-foreground focus:outline-none disabled:cursor-not-allowed disabled:opacity-50',
        local.class,
      )}
      {...rest}
    />
  ...
  return (
    <ComboboxPrimitive.Portal>
      <ComboboxPrimitive.Content
        class={cn(
          'relative z-50 min-w-[8rem] overflow-hidden rounded-md border bg-ui-popover text-ui-popover-foreground shadow-md data-[expanded]:animate-in data-[closed]:animate-out data-[closed]:fade-out-0 data-[expanded]:fade-in-0 data-[closed]:zoom-out-95 data-[expanded]:zoom-in-95 origin-[--kb-combobox-content-transform-origin]',
          local.class,
        )}
        {...rest}
      >
...
export const ComboboxItem = <T extends ValidComponent = 'li'>(
  ...
  return (
    <ComboboxPrimitive.Item
      class={cn(
        'relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-2 pr-8 text-sm outline-none data-[disabled]:pointer-events-none data-[highlighted]:bg-ui-accent data-[highlighted]:text-ui-accent-foreground data-[disabled]:opacity-50',
        local.class,
      )}
      {...rest}
    >
